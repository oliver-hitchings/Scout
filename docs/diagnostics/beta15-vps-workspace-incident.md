...
- While the original remote recovery passphrase appeared unavailable, Beta 16 added host-local passphrase rotation as a safe fallback. Rotation rewraps the same data key, preserves the emergency-key wrapper and encrypted blobs, pushes the header through normal serialized sync, and is accepted only when sync returns `synced` and all three CVs render. The owner subsequently supplied the original passphrase; it was verified locally against the committed recovery header without logging it, so live adoption reused the existing key instead of rotating it.
- Adding the prepared deploy key returned GitHub HTTP `422 key is already in use`. Read-only metadata showed the exact key was already attached to the intended private workspace repository under an older title with write access. No key was removed, duplicated, or broadened; the existing repository-scoped grant was reused.
- Protected adoption run `29833861792` completed successfully. It retained the former VPS workspace as a timestamped rollback sibling, activated the intended private repository, restored encrypted recovery data, reported backup `enabled: true` and `synced`, discovered exactly three CV sources, and rendered exactly three PDFs with managed Typst.
