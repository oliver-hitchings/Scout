...
2. In PowerShell, run `$Installer = Get-Item .\Scout-*-windows-x64.exe`, then `Get-FileHash -LiteralPath $Installer.FullName -Algorithm SHA256`, and compare the result with `checksums.txt`.
